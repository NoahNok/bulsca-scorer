...
class CompetitionSpeedEvent extends Model
{
    ...
    public function getResultQuery()
    {
        ...
        if ($this->getName() == "Swim & Tow") {
            ...
            $results = 'SELECT *, RANK() OVER (ORDER BY points DESC) place FROM (SELECT sr.id, CONCAT(c.name, " ", ct.team) AS team, ct.team AS ctteam, se.name as event, sr.result, sr.disqualification AS disqualification, IF(sr.disqualification IS NOT NULL, 0, IF(sr.result, (1-((sr.result-cse.record)/(cse.record))) * 1000, 0)) AS points FROM speed_results sr INNER JOIN competition_speed_events cse ON cse.id=sr.event INNER JOIN speed_events se ON cse.event=se.id INNER JOIN competition_teams ct ON ct.id=sr.competition_team INNER JOIN clubs c ON c.id=ct.club INNER JOIN leagues l on l.id=ct.league WHERE cse.id=? :league_conds: ORDER BY result) t ORDER BY place;';
        }
