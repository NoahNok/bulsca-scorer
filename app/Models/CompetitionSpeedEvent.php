...
class CompetitionSpeedEvent extends Model implements IEvent, IPenalisable
{
    ...
    public function getResults(): array
    {
        ...
        $scoringVersion = $this->getCompetition->scoring_version;
        ...
        if ($this->getName() == "Swim & Tow") {
            ...
            $results = match ($scoringVersion) {
                '1.0.0' => DB::select('SELECT *, RANK() OVER (ORDER BY points DESC) place FROM (SELECT *, IF( disqualification IS NOT NULL, 0, IF( result_penalties = 0, alpha / 2, IF( result_penalties = 1, alpha + beta / 2, IF( result_penalties = 2, alpha + beta + gamma / 2, IF( result_penalties = 3, alpha + beta + gamma + delta / 2, IF(slowest=fastest, 1000, alpha + beta + gamma + delta + ( 1 -( (result - fastest)/( GREATEST(slowest - fastest, 0) ) ) )*( 1000 -(alpha + beta + gamma + delta)) ) ) ) ) ) ) AS points FROM ( WITH tbl AS ( WITH h AS ( SELECT *, cast( IF( penalties > 0, GREATEST( IF( result > 3, 4 - penalties, result - penalties ), 0 ), result ) AS unsigned ) AS result_penalties FROM ( SELECT sr.event AS event_id, sr.id, CONCAT(c.name, " ", ct.team) AS team, se.name as event, sr.result, sr.disqualification, ( SELECT COUNT(*) FROM speed_result_penalties WHERE speed_result = sr.id ) AS penalties, ( 1000 /( SELECT COUNT(*) FROM speed_results WHERE event = cse.id AND disqualification IS NULL ) ) AS pointsPerTeam, ct.id as tid, sr.result as base_result FROM speed_results sr INNER JOIN competition_speed_events cse ON cse.id = sr.event INNER JOIN speed_events se ON cse.event = se.id INNER JOIN competition_teams ct ON ct.id = sr.competition_team INNER JOIN clubs c ON c.id = ct.club WHERE cse.id = ' . $this->id . ' ORDER BY result ) AS b ) SELECT *, ( SELECT MAX( cast(result as unsigned) ) FROM h WHERE penalties = 0 ) AS slowest, ( SELECT MIN( cast(result as unsigned) ) FROM h WHERE result > 3 AND penalties = 0 ) AS fastest FROM h ) SELECT *, ( pointsPerTeam * ( SELECT COUNT(*) FROM tbl WHERE result_penalties = 0 AND disqualification IS NULL ) ) AS alpha, ( pointsPerTeam * ( SELECT COUNT(*) FROM tbl WHERE result_penalties = 1 AND disqualification IS NULL ) ) AS beta, ( pointsPerTeam * ( SELECT COUNT(*) FROM tbl WHERE result_penalties = 2 AND disqualification IS NULL ) ) AS gamma, ( pointsPerTeam * ( SELECT COUNT(*) FROM tbl WHERE result_penalties = 3 AND disqualification IS NULL ) ) AS delta FROM tbl ) AS b) AS p;'),
                // New scoring as of 2024 AGM Rope Throw Proposal by Anton Oleinik https://www.bulsca.co.uk/resources/view/ec863e58-2f2e-4c75-842e-3d14eb531104
                '1.1.0' => DB::select('SELECT *, RANK() OVER (ORDER BY points DESC) place FROM (SELECT *, IF( disqualification IS NOT NULL, 0, IF( result_penalties = 0, alpha / 2, IF( result_penalties = 1, alpha + beta / 2, IF( result_penalties = 2, alpha + beta + gamma / 2, IF( result_penalties = 3, alpha + beta + gamma + delta / 2, IF(fastest=150000, 1000, IF(slowest=fastest, 1000, alpha + beta + gamma + delta + ( (150000 - result)/(150000 - fastest) )*( 1000 -(alpha + beta + gamma + delta)) ) ) ) ) ) ) ) AS points FROM ( WITH tbl AS ( WITH h AS ( SELECT *, cast( IF( penalties > 0, GREATEST( IF( result > 3, 4 - penalties, result - penalties ), 0 ), result ) AS unsigned ) AS result_penalties FROM ( SELECT sr.event AS event_id, sr.id, CONCAT(c.name, " ", ct.team) AS team, se.name as event, sr.result, sr.disqualification, ( SELECT COUNT(*) FROM speed_result_penalties WHERE speed_result = sr.id ) AS penalties, ( 1000 /(( SELECT COUNT(*) FROM speed_results WHERE event = cse.id AND disqualification IS NULL ) + 5) ) AS pointsPerTeam, ct.id as tid, sr.result as base_result FROM speed_results sr INNER JOIN competition_speed_events cse ON cse.id = sr.event INNER JOIN speed_events se ON cse.event = se.id INNER JOIN competition_teams ct ON ct.id = sr.competition_team INNER JOIN clubs c ON c.id = ct.club WHERE cse.id = ' . $this->id . ' ORDER BY result ) AS b ) SELECT *, ( SELECT MAX( cast(result as unsigned) ) FROM h WHERE penalties = 0 ) AS slowest, ( SELECT MIN( cast(result as unsigned) ) FROM h WHERE result > 3 AND penalties = 0 ) AS fastest FROM h ) SELECT *, ( pointsPerTeam * (( SELECT COUNT(*) FROM tbl WHERE result_penalties = 0 AND disqualification IS NULL )+1) ) AS alpha, ( pointsPerTeam * (( SELECT COUNT(*) FROM tbl WHERE result_penalties = 1 AND disqualification IS NULL )+1) ) AS beta, ( pointsPerTeam * (( SELECT COUNT(*) FROM tbl WHERE result_penalties = 2 AND disqualification IS NULL )+1) ) AS gamma, ( pointsPerTeam * (( SELECT COUNT(*) FROM tbl WHERE result_penalties = 3 AND disqualification IS NULL )+1) ) AS delta FROM tbl ) AS b) AS p;'),
            };
