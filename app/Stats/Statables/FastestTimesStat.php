...
class FastestTimesStat extends Statable
{
    ...
    private string $baseQuery = "SELECT c.name AS competition, c.id AS competition_id, se.name AS event, cl.name AS club, ct.team, t1.time, t1.points, t1.place FROM stats_times t1 JOIN (SELECT event, MIN(time) AS fastest_time FROM stats_times GROUP BY event) t2 ON t1.event=t2.event AND t1.time=t2.fastest_time INNER JOIN
