...
<body class="overflow-x-hidden" data-enable-echo x-data="{
        ...
        this.echo = Echo.join(`judge.competition.{{ \App\DigitalJudge\DigitalJudge::getClientCompetition()->id }}`)
            ...
            .leaving(user => {
                ...
                if (existingUser) {
                    ...
                    existingUser.timeout = setTimeout(() => {
    ...
    @else
        <div class="fixed left-0 bottom-0 w-screen bg-white border-t-2  grid grid-cols-3">
            ...
            <a href="{{ route('dj.dq.issue') }}" class="p-2 text-center border-r">DQ/Penalty</a>
            <a href="{{ route('dj.help') }}" class="p-2 text-center border-r">Help</a>
        </div>
    @endif
