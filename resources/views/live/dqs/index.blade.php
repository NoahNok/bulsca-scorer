...
<body class="overflow-x-hidden flex justify-center w-screen h-screen">
    <div class="w-[90vw] md:w-[70vw] my-12">
        ...
        <a href="{{ route('live') }}" class="link "><span class="text-sm">Back</span></a>
        <br>
        <br>
        <h4>SERCs</h4>
        <div class="flex flex-col mb-2">
            @foreach ($comp->getSercs as $serc)
                <a href="{{ route('live.dqs.event', 'se:' . $serc->id) }}" class="link">{{ $serc->getName() }}</a>
            @endforeach
        </div>
        ...
        <div class="flex flex-col">
            @foreach ($comp->getSpeedEvents as $speed)
                <a href="{{ route('live.dqs.event', 'sp:' . $speed->id) }}" class="link">{{ $speed->getName() }}</a>
            @endforeach
        </div>
