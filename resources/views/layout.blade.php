...
<body class="flex flex-col  overflow-x-hidden" x-data="{ asideCollapsed: false }">
    ...
    <header class="bg-white shadow-sm" x-data="{ navOpen: false }">
        <nav class="mx-auto flex  items-center justify-between p-4 lg:px-8" aria-label="Global">
            <div class="flex lg:flex-1 items-center">
                ...
                <h1 class="ml-3 text-xl font-archivo font-semibold">Scoring.Events</h1>
            </div>
    ...
    <div class="w-full overflow-x-hidden flex">
        ...
        <aside class="  h-screen flex flex-col" :class="asideCollapsed ? 'collapsed' : ''" id="nav">
            <div class="flex flex-row items-center w-full  sm:justify-center  text-white bg-bulsca p-5 h-[8vh]  ">
                ...
                <img src="{{ $brand?->getLogo() ?? 'https://www.bulsca.co.uk/storage/logo/blogo.png' }}"
                    alt="" class="aside-brand-logo hidden w-12  ">
            ...
            <nav class="sidebar " x-data="{ sectionOpen: true }">
                ...
                @isset($brand)
                    ...
                    @if ($brand->isBrandRole(auth()->user(), ['admin', 'welfare']))
                        <div class="section">
                            <p>Brand</p>
                        </div>
                        <div class="section-content">
                            <a href="{{ route('brand.index') }}"
                                class="@if (Str::startsWith(Route::currentRouteName(), 'brand')) selected @endif">
                                <div>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="2" stroke="currentColor" class="w-7 h-7">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            @if ($brand->isBrandRole(auth()->user(), 'admin'))
                                <a href="{{ route('admin.brands.show', $brand) }}"
                                    class="@if (Str::startsWith(Route::currentRouteName(), 'admin')) selected @endif">
                                    <div>
