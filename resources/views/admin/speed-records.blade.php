    ...
    <div class="grid-2">
        <div class="flex flex-col space-y-4">
            ...
            <div class="  relative w-full  ">
                <table editable-table="records" table-submit-csrf="{{ csrf_token() }}"
                    table-after-url="{{ route('admin.records') }}" table-submit-url="{{ route('admin.records.update') }}"
                    ...
                    <tbody>
                        ...
                        @forelse ($events as $event)
                            <tr table-row table-row-owner="{{ $event->id }}" class="bg-white border-b text-right ">
                                ...
                                <td>
                                    <input class="table-input" table-cell table-cell-name="record" placeholder="00:00.000"
                                        type="text"
                                        value="{{ $event->record != null ? sprintf('%02d', $mins) . ':' . str_pad(number_format($secs, 3, '.', ''), 6, '0', STR_PAD_LEFT) : '' }}">
                                </td>
