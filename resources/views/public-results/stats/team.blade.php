...
<head>
    ...
    <title>{{ $club->name }} {{ $team->getTeamLetter() }} | Stats | BULSCA</title>
...
<body class="overflow-x-hidden flex w-full h-full justify-center">
    <div class=" w-full md:w-[75%] m-6 md:my-28 md:mx-0 ">
        ...
        <h1 class="font-bold  " style="font">{{ $club->name }} {{ $team->getTeamLetter() }}</h1>


        <div class="flex  ">Teams:
            @foreach ($distinctTeams as $t)
                <a href="{{ route('public.results.stats.club.team', [$club->name, $t->team]) }}"
                    class="link-stats px-2 first-of-type:ml-1 {{ $t->team == $team->getTeamLetter() ? 'link-stats-selected' : '' }} ">
                    {{ $t->team }}
                </a>
            @endforeach
                        ...
                        <table class="table-auto ">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @foreach ($competedAt as $comp)
                                        <td class="px-2 ">
                                            {{ $placings[0][$comp->id] ?? false ? $nf->format($placings[0][$comp->id]['place']) : '-' }}
                                        </td>
                                    @endforeach
                                </tr>
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        ...
                        {
                            label: '{{ $team->getTeamLetter() }}',
                            data: [
                                @foreach ($competedAt as $comp)
                                    {{ $placings[0][$comp->id] ?? false ? $placings[0][$comp->id]['place'] : 'null' }},
                                @endforeach
                            ],
