...
/**
* @see \App\Http\Controllers\AccountInviteController::resolveNewAccount
* @see app/Http/Controllers/AccountInviteController.php:92
* @route '/invite/{invite}/{email}/resolve/{resolution}/new-acc'
*/
export const resolveNewAccount = (args: { invite: string | { id: string }, email: string | number, resolution: string | number } | [invite: string | { id: string }, email: string | number, resolution: string | number ], options?: RouteQueryOptions): RouteDefinition<'post'> => ({
    url: resolveNewAccount.url(args, options),
    method: 'post',
})

resolveNewAccount.definition = {
    methods: ["post"],
...
* @route '/invite/{invite}/{email}/resolve/{resolution}/new-acc'
*/
resolveNewAccount.url = (args: { invite: string | { id: string }, email: string | number, resolution: string | number } | [invite: string | { id: string }, email: string | number, resolution: string | number ], options?: RouteQueryOptions) => {
    if (Array.isArray(args)) {
        args = {
            invite: args[0],
            email: args[1],
    ...
    return resolveNewAccount.definition.url
            .replace('{invite}', parsedArgs.invite.toString())
            .replace('{email}', parsedArgs.email.toString())
            .replace('{resolution}', parsedArgs.resolution.toString())
            .replace(/\/+$/, '') + queryParams(options)
...
* @route '/invite/{invite}/{email}/resolve/{resolution}/new-acc'
*/
resolveNewAccount.post = (args: { invite: string | { id: string }, email: string | number, resolution: string | number } | [invite: string | { id: string }, email: string | number, resolution: string | number ], options?: RouteQueryOptions): RouteDefinition<'post'> => ({
    url: resolveNewAccount.url(args, options),
    method: 'post',
})

const AccountInviteController = { show, resolve, resolveNewAccount }

export default AccountInviteController